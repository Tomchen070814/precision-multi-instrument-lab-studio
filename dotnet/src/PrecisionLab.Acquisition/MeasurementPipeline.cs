...
public sealed class MeasurementPipeline : BackgroundService, IMeasurementPublisher
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            ...
            {
                await _sink.WriteAsync(measurement, CancellationToken.None).ConfigureAwait(false);
                _hub.Publish(measurement);
            }
        }
        catch (Exception exception)
        {
            _logger.LogCritical(exception, "The durable measurement pipeline stopped unexpectedly.");
            throw;
        }
        finally
        {
